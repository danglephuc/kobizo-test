...
import { Loading, Dialog, Button, Form, Input, MessageBox } from 'element-react';
                ...
                <Dialog.Body>
                    <Form ref={formRef} className="en-US" model={formData} labelWidth="120" rules={rules}>
                        <Form.Item label="Key" prop="key">
                            <Input value={formData.key} onChange={onChange.bind(this, 'key')}/>
                        </Form.Item>
                        <Form.Item label="Value" prop="value">
                            <Input value={formData.value} onChange={onChange.bind(this, 'value')}/>
                        </Form.Item>
                    </Form>
                </Dialog.Body>
                <Dialog.Footer className='dialog-footer'>
                    <Button onClick={() => {closeModal(null)}}>Cancel</Button>
                    <Button type='primary' onClick={onSubmit}>{ meta ? 'Update' : 'Create' }</Button>
                </Dialog.Footer>
            </Loading>
        </Dialog>
    );
